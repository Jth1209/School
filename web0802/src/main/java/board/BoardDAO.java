...
public class BoardDAO {
	...
	private String BOARD_LIST = "select * from board";//테이블 모든 정보를 불러오는 쿼리
	private String BOARD_INSERT = "insert into board(writer,title,content,regtime,hits) values (?,?,?,now(),0)";//테이블에 정보를 추가해주는 쿼리
	private String BOARD_VIEW = "select * from board where num = ?";//선택한 테이블의 정보를 확인할 수 있는 쿼리
	private String BOARD_HITS = "update board set hits = hits + 1 where num = ?";//선택한 정보를 몇번 선택했는지 확인할 수 있는 쿼리
	private String BOARD_DELETE = "delete from board where num = ?";//선택한 테이블의 정보를 삭제하는 쿼리
	private String BOARD_UPDATE = "update board set writer = ?, title = ?, content = ?, regtime = now() where num = ?";//선택한 테이블의 값을 수정하는 쿼리
